...
3. **Firmas EdDSA:** Se utilizará criptografía Twisted Edwards Curve (Ed25519) para firmar los *Tokens de Sesión (JWT)*, proporcionando el estándar más avanzado de la industria.
...
- `security/aes/aes_gcm.py`: Clase utilitaria en Python para cifrar y descifrar usando AES-256-GCM.
- `security/argon2/argon_hasher.py`: Implementación de Argon2id para el hasheo de contraseñas.
- `security/eddsa/eddsa_signer.py`: Generación de firmas digitales usando Ed25519 para validar Tokens de Sesión.
...
1. **Scripts Unitarios Locales:** Se creará un script principal de prueba (`security/test_crypto.py`) que:
   ...
   - Firme y verifique un token (EdDSA).
